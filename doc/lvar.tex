...
We write $\tm{B}\sqcupB\tm{B'}$ for the maximum on status bits, which is Boolean disjunction. We write $\tm{s}\sqcups\tm{s'}$ for the pointwise maximum on states and $\tm{S}\sqcupS\tm{S'}$ for the pointwise maximum on stores. The latter is only defined if the structure of $\tm{S}$ and $\tm{S'}$ is identical up to domain values:
\[
\setlength{\arraycolsep}{2pt}
\begin{array}{lclcl}
