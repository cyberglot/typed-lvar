\documentclass{article}

% use for separate compilation
\usepackage{subfiles}
\newcommand{\onlyinsubfile}[1]{#1}
\newcommand{\notinsubfile}[1]{}

% bibtex setup
% (most conferences require a bibtex style which is incompatible with biber,
% leaving you to redo all the nice complex \textcite commands by hand)
\usepackage[%
  backend=biber,
  bibencoding=utf8,
  style=authoryear,
  sorting=nyt,
  sortcites=true,
  natbib=true,
  backref=false,
  backrefstyle=all+,
  hyperref=true,
]{biblatex}
\bibliography{main.bib}

% gives you \ie, \eg, \etc
\usepackage[all]{foreign}

% mathematical symbol packages go here
\usepackage[fleqn]{amsmath}
\usepackage{amssymb}
\usepackage{stmaryrd}
\usepackage{mathtools}
\usepackage{textgreek}

% useful for defining commands to use in text mode
\usepackage{xspace}
\newcommand{\lambdaLVar}[0]{$\lambda_{LVar}$\xspace}
\newcommand{\lambdalvar}[0]{$\lambda_{lvar}$\xspace}

% inference rules
\usepackage{mathpartir}

% landscape pages using \begin{landscape}\end{landscape}
\usepackage{pdflscape}

% gotsta have a todo command
\newcommand{\remember}[1]{\todo[color=pink!40]{#1}}

% theorem environments
% (you can do way fancier theorems but they usually break when you have to
% convert your work to use one of the ancient latex conference templates)
\usepackage{amsthm}

% personally I~hate it when some math mode shit starts immediately after the
% thing that says `theorem 3', but your mileage may vary -- regardless, this
% is a good bit of documentation
\newtheoremstyle{break}% name
  {}%          Space above, empty = `usual value'
  {}%          Space below
  {\itshape}%  Body font
  {}%          Indent amount (empty = no indent, \parindent = para indent)
  {\bfseries}% Thm head font
  {.}%         Punctuation after thm head
  {\newline}%  Space after thm head: \newline = linebreak
  {}%          Thm head spec

\theoremstyle{break}
\newtheorem{theorem}{Theorem}
\newtheorem{corollary}{Corollary}
\newtheorem{lemma}{Lemma}

\theoremstyle{definition}
\newtheorem{definition}{Definition}
\newtheorem{conjecture}{Conjecture}

% this one is for use within a proof environment
\theoremstyle{remark}
\newtheorem*{case}{Case}
\newtheorem*{subcase}{Sub-case}

% sort macros
\usepackage[dvipsnames]{xcolor}
\newcommand{\tm}[1]{\ensuremath{{\color[HTML]{E15CFF}#1}}}
\newcommand{\ty}[1]{\ensuremath{{\color[HTML]{6e4cef}#1}}}
\newcommand{\cs}[1]{\ensuremath{{\color[HTML]{009180}#1}}}
\newcommand{\tmty}[2]{\ensuremath{\tm{#1}:\ty{#2}}}

% greyed out
\newcommand{\deprecatedstyle}{%
  \renewcommand{\tm}[1]{\ensuremath{{\color[HTML]{C0C0C0}##1}}}%
  \renewcommand{\ty}[1]{\ensuremath{{\color[HTML]{C0C0C0}##1}}}%
  \renewcommand{\cs}[1]{\ensuremath{{\color[HTML]{C0C0C0}##1}}}%
  \color[HTML]{C0C0C0}}
\newcommand{\deprecate}[1]{{\deprecatedstyle#1}}

% definitions
\newcommand{\sep}{\;\mid\;}

% environments and hyper-environments
\newcommand{\emptyenv}{\cdot}

% common operations
\newcommand{\subst}[4][]{\ifstrempty{#1}{\ensuremath{#2\{#3/#4\}}}{\ensuremath{#2(\{#3/#4\}\cup#1)}}}
\newcommand{\plug}[2]{\ensuremath{#1[#2]}}
\DeclareMathOperator{\fv}{fv}
\DeclareMathOperator{\dom}{dom}
\DeclareMathOperator{\cod}{cod}

% use for namespaces
\usepackage{namespc}

% namespace declaration
% open globally with \usingnamespace{llam}, locally with \llam{}
\namespace*{llam}{
  % types
  \newcommand{\tyunit}[0]{\ensuremath{\ty{\mathbf{1}}}}
  \newcommand{\tyvoid}[0]{\ensuremath{\ty{\mathbf{0}}}}
  \newcommand{\typrod}[2]{\ensuremath{{#1}\mathbin{\ty{\times}}{#2}}}
  \newcommand{\tysum}[2]{\ensuremath{{#1}\mathbin{\ty{+}}{#2}}}
  \newcommand{\tylolli}[2]{\ensuremath{{#1}\mathbin{\ty{\multimap}}{#2}}}
  \newcommand{\tyfun}[2]{\ensuremath{{#1}\mathbin{\ty{\rightarrow}}{#2}}}
  \newcommand{\tyD}[1]{\ensuremath{\ty{\mathcal{D}^{#1}}}}
  \newcommand{\tyL}[1]{\ensuremath{\ty{\mathcal{L}_{\mathcal{D}}^{#1}}}}
  \newcommand{\tyJ}[0]{\ensuremath{\ty{\mathcal{J}}}}
  % terms
  \newcommand{\andthen}[2]{\ensuremath{#1;#2}}
  \newcommand{\letbind}[3]{\ensuremath{\mathbf{let}\;#1\mathbin{=}#2\;\mathbf{in}\;#3}}
  \newcommand{\pair}[2]{\ensuremath{(#1,#2)}}
  \newcommand{\letpair}[4]{\ensuremath{\letbind{\pair{#1}{#2}}{#3}{#4}}}
  \newcommand{\labinl}[0]{\ensuremath{\mathbf{inl}}}
  \newcommand{\labinr}[0]{\ensuremath{\mathbf{inr}}}
  \newcommand{\inl}[1]{\ensuremath{\labinl\;#1}}
  \newcommand{\inr}[1]{\ensuremath{\labinr\;#1}}
  \newcommand{\casesum}[5]{\ensuremath{\mathbf{case}\;#1\;\left\{\inl{#2}\mapsto{#3};\;\inr{#4}\mapsto{#5}\right\}}}
  \newcommand{\unit}[0]{\ensuremath{()}}
  \newcommand{\letunit}[2]{\ensuremath{\letbind{\unit}{#1}{#2}}}
  \newcommand{\absurd}[1]{\ensuremath{\mathbf{absurd}\;#1}}
  \newcommand{\lvarnew}[0]{\ensuremath{\mathbf{new}}}
  \newcommand{\lvarput}[0]{\ensuremath{\mathbf{put}}}
  \newcommand{\lvarget}[0]{\ensuremath{\mathbf{get}}}
  \newcommand{\lvarfreeze}[0]{\ensuremath{\mathbf{freeze}}}
  \newcommand{\lvarrun}[0]{\ensuremath{\mathbf{runLVar}}}
  \newcommand{\lvarerror}[0]{\ensuremath{\mathbf{error}}}
  \newcommand{\sts}[2]{\ensuremath{\langle {#1},\;{#2} \rangle}}
  \newcommand{\config}[2]{\ensuremath{\langle {#1}\;\vert\;{#2} \rangle}}
  \newcommand{\confige}[2]{\ensuremath{\langle E\lbrack {#1} \rbrack \; \vert\;{#2} \rangle}}
  \newcommand{\configl}[2]{\ensuremath{\langle {#1} \; ; \;{#2} \rangle}}
  % evaluation contexts
  \newcommand{\hole}[0]{\ensuremath{\square}}
  % typing judgements
  \newcommand{\seq}[3]{\ensuremath{#1\vdash\tmty{#2}{#3}}}
  % reduction relations
  \newcommand{\red}[0]{\ensuremath{\longrightarrow}}
  \newcommand{\rede}[0]{\ensuremath{\longrightarrow_{E}}}

  % simplifying terminology usually repeated verbetim
  \newcommand{\lvar}[0]{\ensuremath{\lambda_{\text{LVar}}}}
  \newcommand{\typedlvar}[0]{\ensuremath{\text{Typed } \lambda_{\text{lvar}}}}
  \newcommand{\mspc}[0]{\ensuremath{\;}}
}{}
\newcommand{\llam}[1]{\namespace*{llam}{}{#1}}

% load cleverref
\usepackage{varioref}
\usepackage{hyperref}
\usepackage{cleveref}

% load todonotes
\usepackage{todonotes}

\begin{document}
\renewcommand{\onlyinsubfile}[1]{}
\renewcommand{\notinsubfile}[1]{#1}

\subfile{lvar}

\printbibliography

\end{document}