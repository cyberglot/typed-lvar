...
We write $\tm{s}\sqcups\tm{s'}$ for the pointwise maximum on states and $\tm{S}\sqcupS\tm{S'}$ for the pointwise maximum on stores. The latter is only defined if the structure of $\tm{S}$ and $\tm{S'}$ is identical \emph{up to location names}:
\[
\setlength{\arraycolsep}{2pt}
\begin{array}{lclcl}
  \tm{\st{d}{\frz}} & \sqcups & \tm{\st{d'}{\frz'}}
  & = & \tm{\st{d \sqcupD d'}{\frz \sqcupB \frz'}} \\
  \tm{\storeemp}          & \sqcupS & \tm{\storeemp}            & = & \tm{\storeemp} \\
  \tm{\storeext{S}{l}{p}} & \sqcupS & \tm{\storeext{S'}{l}{p'}} & = & \tm{\storeext{S \sqcupS S'}{l}{p \sqcups p'}}
\end{array}
\]
...
\begin{theorem}[Subject reduction, $\red$]
  \hfill\\
