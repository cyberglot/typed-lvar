...
\begin{lemma}[Error-freedom, $\red$]
  \label{lem:error-free}
  If $\seq{\ty{\Gamma}}{C}{T}$, then $\tm{C}\centernot\red\tm{\error}$.
\end{lemma}
\begin{proof}
  By inversion on the structure of $\tm{C}\red\tm{\error}$.
  \todo{%
...
\begin{theorem}[Error-freedom, $\rede$]
  ...
  If $\seq{\ty{\Gamma}}{C}{T}$, then $\tm{C}\centernot\rede\tm{\error}$.
\end{theorem}
\begin{proof}
...
\end{theorem}
\begin{proof}
  By induction on the structure of $\seq{\ty{\Gamma}}{C}{T}$.
  \todo{%
